...
|---|---|
| **Модель** | `tiny`/`base`/`small`/`medium`/`large-v3`/`large-v3-turbo` — больше = точнее, но медленнее. |
...
| **Beam** | `1` — быстрее, `5` — точнее (по умолчанию). |
| **Фильтр тишины (VAD)** | Обрезает паузы — меньше «галлюцинаций» на тишине. |
| **Автопунктуация** | Подсказывает модели ставить запятые, точки и заглавные буквы. |
| **Голосовые команды** | Скажите «запятая», «точка», «новая строка» — вставится знак. По умолчанию выключено. |
| **Свой словарь** | Имена и термины через запятую — модель будет их узнавать. |
